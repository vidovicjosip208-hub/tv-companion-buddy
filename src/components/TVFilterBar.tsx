...
const TVFilterBar = ({ focusedFilter }: TVFilterBarProps) => {
  ...
  return (
    <div className="flex gap-2 sm:gap-3 mb-4 sm:mb-6 mt-1 sm:mt-2">
      {filters.map((filter, index) => (
        <button
          key={filter}
          className={cn(
            "px-3 sm:px-5 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 border",
            focusedFilter === index
