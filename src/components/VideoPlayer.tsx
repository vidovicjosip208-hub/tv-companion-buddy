...
// Proširena lista za pretragu po broju (dodaj sve kanale ovdje)
const ALL_CHANNELS: { num: number; label: string }[] = [
  { num: 4, label: "federalna" },
  { num: 5, label: "RTRS" },
  { num: 6, label: "MAX" },
  { num: 7, label: "Hayat TV" },
  { num: 8, label: "OBN" },
  { num: 9, label: "Nova TV" },
  { num: 10, label: "HRT 1" },
];
...
const VideoPlayer = ({
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      // ── Digit input → switch to favorite channel by number ─────────────
      if (/^\d$/.test(e.key)) {
        e.preventDefault();
  ...
  // Derive found channel for overlay
  ...
  return (
    ...
    >
      <div className="relative w-full h-full overflow-hidden">
        {/* Backdrop image – hidden once video stream is playing */}
