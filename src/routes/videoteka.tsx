...
export const Route = createFileRoute("/videoteka")({
  head: () => ({
    meta: [
      { title: "Videoteka — LovableTV" },
      { name: "description", content: "Kolekcija filmova i serija." },
    ],
  }),
  component: Videoteka,
});
...
function Videoteka() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-gradient-to-br from-slate-950 via-black to-slate-900 p-8 text-white">
      <Film className="h-16 w-16 text-amber-400" />
      <h1 className="text-4xl font-bold">Videoteka</h1>
      <p className="text-white/60">Filmovi i serije — uskoro.</p>
      <Link
        to="/"
